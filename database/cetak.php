...
$id = $_GET['id'];
$show = mysql_query("SELECT * FROM db_mahasiswa WHERE id='$id'");
...
$sql=mysql_query("SELECT * FROM db_mahasiswa WHERE id='$id'");
$data=mysql_fetch_array($sql);
$pdf->setXY(25,20);
$pdf->cell(30,6,'NIM');$pdf->cell(50,6,': '.$data['nim']);
...
$row=6;
$y=$y_initial+$row;
$nilai=mysql_query("select * from db_mahasiswa where id='$id'");
while ($daftar=mysql_fetch_array($nilai)) {
	...
	$pdf->setY(47);
	$pdf->setX(50);
	$pdf->cell(50,6,'Analisis Perancangan Perangkat Lunak',1,0,'C',1);
	$pdf->setY(47);
	$pdf->setX(100);
